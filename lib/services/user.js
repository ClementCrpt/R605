...
module.exports = class UserService extends Service {
	...
	async create(user) {
		...
		try {
			...
			mailerService.sendMail({
				...
				subject: 'Welcome to HelloCiné',
				text: MailBody.newUserText(newUser.username),
				html: MailBody.newUserHtml(newUser.username),
			});
	...
	async delete(user_id) {
		...
		try {
			...
			await Favorite.query().where({ user_id }).del();
